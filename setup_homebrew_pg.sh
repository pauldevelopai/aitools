...
$PG_BIN/createdb grounded 2>/dev/null || echo "Database already exists"
$PG_BIN/psql -d grounded -c "CREATE USER grounded WITH PASSWORD 'changeme';" 2>/dev/null || echo "User already exists"
$PG_BIN/psql -d grounded -c "GRANT ALL PRIVILEGES ON DATABASE grounded TO grounded;"
$PG_BIN/psql -d grounded -c "CREATE EXTENSION IF NOT EXISTS vector;"
$PG_BIN/psql -d grounded -c "GRANT ALL ON SCHEMA public TO grounded;"

# Test connection
echo ""
echo "Testing connection..."
PGPASSWORD=changeme $PG_BIN/psql -h localhost -U grounded -d grounded -c "SELECT version();"
