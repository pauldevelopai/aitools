"""Post-run quality assessment for Brain-created records.

After each Brain mission completes, this module reviews the created records,
scores them on completeness criteria, and auto-rejects low-quality entries.

Migrated from app/workflows/agent/quality_judge.py to work with the Brain.
"""
...
def _judge_lesson(db: Session, name: str) -> dict[str, Any]:
    """Score a Lesson record."""
    from app.models.lessons import Lesson

    lesson = db.query(Lesson).filter(Lesson.title.ilike(name)).first()
    if not lesson:
        return {"score": 0, "quality": "low", "issues": ["record not found"], "checks_passed": 0, "checks_total": 5}
    ...
    if lesson.content_markdown and len(lesson.content_markdown.strip()) > 200:
        passed += 1
    else:
        issues.append("content_markdown too short or missing (< 200 chars)")

    if lesson.learning_objectives and len(lesson.learning_objectives) >= 2:
        passed += 1
    else:
        issues.append("fewer than 2 learning objectives")

    if lesson.task_prompt and len(lesson.task_prompt.strip()) > 20:
        passed += 1
    else:
        issues.append("task_prompt too short or missing")

    if lesson.verification_type in ("self_report", "ai_review"):
        passed += 1
    else:
        issues.append("invalid verification_type")
    ...
    return {
        ...
        "checks_passed": passed,
        "checks_total": checks,
        "record_id": str(lesson.id) if lesson else None,
    }
...
async def judge_mission_results(
    ...
    """Assess quality of all records created by a Brain mission run.
...
def _auto_reject(db: Session, tool_name: str, name: str) -> None:
    ...
    try:
        if tool_name == "create_media_organization":
            ...
            from app.models.lessons import Lesson
            record = db.query(Lesson).filter(Lesson.title.ilike(name)).first()
            if record:
